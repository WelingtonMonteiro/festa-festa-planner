...
const ProductCard = ({ product, onEdit, onDelete }: ProductCardProps) => {
  ...
  // ROI calculation for theme products
  const calcROI = (product: Product) => {
    ...
    const totalRevenue = (product.rentCount || 0) * averagePrice;
  ...
  return (
    <Card key={productId} className={cn(
      ...
      <CardContent>
        ...
        {product.type === 'kit' && product.items && product.items.length > 0 && (
          <div className="mb-3">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          <div className="bg-muted rounded p-2">
            ...
            <div>{product.rentCount || 0} {(product.rentCount || 0) === 1 ? 'vez' : 'vezes'}</div>
          </div>
